...
### Problème
- La couleur primaire `#3eb9a1` est hardcodée dans 5 composants (Header, RecipeCard, RecipeDetail, SearchRecipes, RecipeForm)
...
### Fichiers impactés
- `src/style.css` — palette `@theme` à recalculer + nettoyer les doublons `:root` (les variables `--primary-*` et `--surface-*` en format RGB sont dupliquées entre le bloc `@theme` et le bloc `:root`)
- `src/assets/base.css` — nettoyage variables `--vt-c-*` inutilisées
...
4. **Retirer les `overflow-y-auto`** des divs de sections individuelles (Catégories, Difficulté, Coût, Tags) — chacune a actuellement son propre scroll, ce qui cause les scrollbars multiples
...
- Le lien `← Recettes` ramène toujours à `/` (pas de préservation de filtres — simplification, les filtres sont des query params éphémères)
- Le segment catégorie est cliquable et pointe vers `/?categoryCode=[CODE]` (ex: `/?categoryCode=PLAT`)

### Implémentation
- Créer un composant `BreadcrumbNav.vue` réutilisable (nommé ainsi pour éviter toute confusion avec le composant PrimeVue `Breadcrumb`)
...
### Fichiers impactés
- `src/components/BreadcrumbNav.vue` — nouveau composant
...
- La suppression d'image en mode édition met le champ `thumbnail` à `null` au save — le backend gère déjà le nettoyage du fichier serveur via le service existant
...
- Utilisation de `vue-draggable-plus` (wrapper Vue 3 activement maintenu pour SortableJS, compatible Composition API)
...
- Bouton submit désactivé tant que les champs requis ne sont pas remplis (`title`, `categoryCode`, `difficultyCode`, `costCode`, `servings` non vides + au moins un ingrédient avec un `name` non vide)

### Fichiers impactés
- `src/components/RecipeForm.vue` — upload stylé, drag-and-drop, validation
- `package.json` — ajout dépendance `vue-draggable-plus`
...
- `vue-draggable-plus` (wrapper Vue 3 pour SortableJS, compatible Composition API)
